...
/*   Updated: 2024/01/11 20:31:04 by yzaoui           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	find_file_rd(t_node *n, int nbr_rd, int limit)
{
	...
	ft_strcpm(n->str, "<") || ft_strcpm(n->str, "<<"))
		return (-7);
	nbr_rd++;
	if (limit && n->type_input == NON_DEFINI)
		n->type_input = F_RD2;
...
static int	find_all_rd(t_node *n, t_type_input previous, int nbr_rd)
{
	...
	{
		n->type_input = R_OUT;
		return (find_file_rd(n->next_node, nbr_rd, 0));
	}
	else if (n->type_input == NON_DEFINI && ft_strcpm(n->str, ">>"))
	{
	...
	else if (n->type_input == NON_DEFINI && ft_strcpm(n->str, "<<"))
	{
		...
		return (find_file_rd(n->next_node, nbr_rd, 1));
	}
	if (n->type_input != SEPARATOR)
		previous = n->type_input;
